...
n=5; % hur nogrant den ska simulera studs i väggar etc. 5 räcker
r=0.8; %reflection coefficient for the walls, in general -1<R<1. nära 0 ger ingen reflektion
%r=0.8 är skethögt!!
rm=[20 20 3]; %row vector giving the dimensions of the room. 20x20 bottenarea, 3m takhöjd
...
ideal_response=rir(fs, mic, n, 0, rm, src); % ett perfekt impulssvar utan eko
...
cvx_begin
 ...
 minimize( norms(H.*Q_start - ideal_response_fft) + 1e-6*lambda*norm(Q_start) )
cvx_end

%% Testing filter in both domains
Y=X.*Q_start.*H; %applying filter

y=real(ifft(Y,Ly2));
y=y(1:Ly);
y=y/max(abs(y)); %normalizing

q=ifft(Q_start,Ly2);
q=q(1:Ly/8);
y_time_domain=filter(q,1,x); %applying filter
y_time_domain=filter(h,1,y_time_domain); %applying the rir
y_time_domain=y_time_domain/max(abs(y_time_domain));%normalizing

sound(y,fs);

diff=y(1:length(y_time_domain))-y_time_domain;
...
cvx_begin
 ...
 minimize(norm( mean( sum(H.*Q_start ,1)'-ideal_response_fft,1)) )
