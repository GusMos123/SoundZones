...
Ly=length(x)+length(h)-1;  % Find length of convolution
...
%% Listening to impuolse response + song

selected_mic=1; % Mic to listen to
...
H_bright=squeeze(H(1:mics_per_zone,:,:));
H_dark=squeeze(H(mics_per_zone+1:end,:,:));

%% Optimization
mu=0.3; % Param for how much dark zone should matter

cvx_solver_settings( 'max_iter', 2 )
cvx_begin
  
 variable Q_topgun(16,Ly2) % One filter per speaker (16)
 minimize(goal_function(Q_topgun, H_bright, H_dark, ideal_response_fft_bright, mu))
cvx_end

%% Listening to filter + impulse response + song
desired_mic=3; % Mic to listen to
...
mic=1; % Mic we want to listen to
